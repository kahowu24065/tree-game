...
import { SPECIES, STAGE_NAMES, speciesForSeason, stageIndexFor, stageSampleCm } from '../src/data/species';
...
describe('樹種', () => {
  ...
  it('真實高度對應賽季目標（20／50／100 米）', () => {
    const typicalTop = (t: string) => Math.max(...(t.match(/\d+(\.\d+)?/g) ?? ['0']).map(Number));
    for (const sp of SPECIES) {
      const target = SEASONS.find((s) => s.id === sp.season)!.targetCm / 100;
      // The usual or record height has to reach the season target, and no species is wildly off.
      expect(Math.max(sp.maxM, typicalTop(sp.typicalM))).toBeGreaterThanOrEqual(target * 0.95);
      expect(typicalTop(sp.typicalM)).toBeGreaterThanOrEqual(target * 0.7);
      expect(typicalTop(sp.typicalM)).toBeLessThanOrEqual(target * 1.6);
    }
    const s12 = speciesForSeason('s12');
    for (const sp of s12) expect(sp.maxM).toBeGreaterThanOrEqual(95);
  });
  ...
  it('生長階段跟賽季目標高度', () => {
    ...
    // 20 m in a 3-month game is a 巨樹, but only a 青年樹 in a 1-year game.
    expect(stageIndexFor(2000, 2000)).toBe(4);
    expect(stageIndexFor(2000, 10000)).toBe(2);
  });
