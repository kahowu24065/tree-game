import{ct as e,dt as t,nt as n,sn as r,st as i,ut as a,v as o,vt as s}from"./ui-D_HieZpC.js";var c=[[`自動（跟天氣）`,null],[`平靜`,.05],[`微風`,.25],[`強風`,.55],[`暴雨`,.7],[`颱風`,1]],l=`<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 5.5a4 4 0 0 0 4.9 4.9l1.1 1.1-2.2 2.2-1.1-1.1-7.6 7.6a2 2 0 0 1-2.8-2.8l7.6-7.6-1.1-1.1 2.2-2.2z" fill="currentColor" fill-opacity=".18"/></svg>`,u=[[`health`,`健康 H`],[`moisture`,`水分 W`],[`nutrients`,`養分 N`],[`resist`,`抗風力 R`]];function d(d,f){d.hidden=!1,d.innerHTML=`<button type="button" class="dev-fab glass" data-dev="toggle" aria-label="開發者面板">${l}</button><section class="dev-panel glass" hidden></section>`;let p=d.querySelector(`.dev-panel`),m=!1,h=null,g=``,_=()=>{let e=f.ecoCaps();return e?`動物上限（${t[e.stage]}）：${e.groups} 組・${e.members} 隻・體型最大「${e.maxSize}」；而家訪客 ${e.visitorGroups} 組 ${e.visitorMembers} 隻${e.residentGroups?`，另長駐 ${e.residentGroups} 組`:``}`:`動物上限：（場景未準備好）`},v=()=>`畫面上：${f.ecoInfo().map(e=>`${e.name}×${e.count}${e.resident?`（長駐）`:``}`).join(`、`)||`冇`}`,y=()=>{let l=f.dev();if(p.hidden=!l.open,d.classList.toggle(`open`,l.open),!l.open||m)return;let y=f.state(),b=f.countdown(),x=e=>`<button type="button" class="${l.events.includes(e)?`on`:``}" data-dev-event="${e}" ${l.mode===`real`?`disabled`:``}>${s(n(e))}<small>${r[e].damage?`−${r[e].damage}`:`0`}</small></button>`,S=e=>`<option value="${e}" ${l.forecast?.event===e?`selected`:``}>${s(n(e))}</option>`,C=l.forecast?Math.max(0,(l.forecast.at-Date.now())/36e5):6;p.innerHTML=`
      ...
      <p class="dev-note">${l.mode===`real`?`真實：${s(f.liveEvents().map(e=>n(e)).join(`、`)||`晴天／多雲`)}`:`可以揀多個警告：熱、雨、風三類會疊加（同類只計最嚴重）。揀酷熱／暴雨／黑雨會即刻計水分（每日每樣一次），仲會出應急行動掣；揀寒冷會開「保暖」。`}</p>
      <div class="dev-events">${f.events.map(x).join(``)}</div>
      <div class="dev-row">
        <label>12 小時預報
          <select data-dev-fc-event ${l.mode===`real`?`disabled`:``}><option value="">（冇）</option>${f.events.filter(e=>r[e].severe).map(S).join(``)}</select>
        </label>
        <label>幾耐後 <input type="number" min="0" max="12" step="0.5" value="${C.toFixed(1)}" data-dev-fc-hours ${l.mode===`real`?`disabled`:``}/> 小時</label>
      </div>
      <p class="dev-note">倒數：${b?`${s(n(b.event))} · ${b.active?`生效中`:`${b.hours.toFixed(1)} 小時後`}（${s(b.source)}）`:`冇`}</p>
      ...
      <div class="dev-actions">
        ...
        <button type="button" data-dev="fx-replay" ${y.lastCollapse?``:`disabled`}>重播倒塌</button>
        <button type="button" data-dev="fx-dying">瀕死</button>
        <button type="button" class="danger" data-dev="fx-death">枯死（動畫）</button>
      </div>
      <p class="dev-note">倒塌 ${y.collapses||0}/2${y.doubleRDate?`・雙倍加固日 ${s(y.doubleRDate)}`:``}${y.doubleRPending?`・雙倍加固待開始`:``}・高度 ${Math.round(y.heightCm)} 厘米・樹齡 ${y.ageDays||0} 日・里程碑 ${Object.keys(y.milestones??{}).join(`、`)||`冇`}</p>
      <h4 class="dev-h">樹種・生長階段預覽</h4>
      <div class="dev-row">
        <label>樹種 <select data-dev-species><option value="">（存檔：${s(a.find(e=>e.id===y.species)?.name??``)}）</option>${a.map(e=>`<option value="${e.id}" ${l.preview.species===e.id?`selected`:``}>${s(e.name)}（紀錄 ${e.targetM} 米）</option>`).join(``)}</select></label>
        <label>階段 <select data-dev-stage><option value="">（跟高度）</option>${t.map((e,t)=>`<option value="${t}" ${l.preview.stage===t?`selected`:``}>${t+1}. ${e}</option>`).join(``)}</select></label>
        <label>島嶼 <select data-dev-island><option value="">（跟樹）</option>${t.map((e,t)=>`<option value="${t}" ${l.preview.island===t?`selected`:``}>${t+1}. ${e}島</option>`).join(``)}</select></label>
      </div>
      <p class="dev-note" data-dev-habitat>${s(f.habitatInfo())}</p>
      ...
      <div class="dev-row">
        <label>召喚 <select data-dev-animal>${i.map(t=>`<option value="${t.id}" ${t.id===g?`selected`:``}>${s(e[t.category])}・${s(t.name)}</option>`).join(``)}</select></label>
